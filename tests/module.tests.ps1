...
process{
    ...
    Describe "New-LogMessage" -Tag 'build' {
        Context 'Create log' {
            $TestLog = (New-TemporaryFile).FullName
            $returnedData = New-LogMessage -Path $TestLog -message 'Hello Pester!'

            It 'gets result' {
                $returnedData|Should BeNullOrEmpty
            }
            It 'log file content should exist' {
                Get-Content -Path $TestLog | Should not BeNullOrEmpty
            }
            It 'log file content check' {
                Get-Content -Path $TestLog | Select-String -Pattern 'Hello Pester!'
            }

            # Delete test log
            Remove-Item -Path $TestLog
        }
    }
}